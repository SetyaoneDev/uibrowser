...
import newuibrowser.Entitlement
import newuibrowser.R
import acr.browser.lightning.browser.sessions.Session
import newuibrowser.extensions.snackbar
...
import newuibrowser.Component
import newuibrowser.utils.isBookmarkUrl
import newuibrowser.utils.isDownloadsUrl
import newuibrowser.utils.isHistoryUrl
import newuibrowser.utils.isIncognitoPageUrl
import newuibrowser.utils.isSpecialUrl
import newuibrowser.utils.isStartPageUrl
import newuibrowser.view.BookmarkPageInitializer
import newuibrowser.view.DownloadPageInitializer
import newuibrowser.view.FreezableBundleInitializer
import newuibrowser.view.HistoryPageInitializer
import newuibrowser.view.HomePageInitializer
import newuibrowser.view.IncognitoPageInitializer
import newuibrowser.view.NoOpInitializer
import newuibrowser.view.TabInitializer
import newuibrowser.view.WebPageTab
...
class TabsManager @Inject constructor(
    ...
    */
    ...
    {
        val bundle = newuibrowser.utils.FileUtils.readBundleFromStorage(application, aFilename)
    ...
    fun renameSession(aOldName: String, aNewName: String) {
        ...
        newuibrowser.utils.FileUtils.renameBundleInStorage(application, fileNameFromSessionName(oldName), fileNameFromSessionName(aNewName))
    ...
    private fun saveCurrentSession(aName: String) {
        ...
        iScopeThreadPool.launch {
            ...
            newuibrowser.utils.FileUtils.writeBundleToStorage(application, outState, temp)
            // Defensively delete session backup, that should never be needed really
            newuibrowser.utils.FileUtils.deleteBundleInStorage(application, backup)
            // Rename our session file as backup
            newuibrowser.utils.FileUtils.renameBundleInStorage(application, session, backup)
            // Rename our temporary session to actual session
            newuibrowser.utils.FileUtils.renameBundleInStorage(application, temp, session)
...
//            }
     ...
     * browser next starts.
     */
    fun clearSavedState() = newuibrowser.utils.FileUtils.deleteBundleInStorage(application, FILENAME_SESSION_DEFAULT)
    ...
    fun deleteSession(aSessionName: String) {
        ...
        // Delete session file
        newuibrowser.utils.FileUtils.deleteBundleInStorage(application, fileNameFromSessionName(iSessions[index].name))
