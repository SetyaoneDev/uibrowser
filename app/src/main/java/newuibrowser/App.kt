...
import newuibrowser.activity.IncognitoActivity
import newuibrowser.database.bookmark.BookmarkExporter
import newuibrowser.database.bookmark.BookmarkRepository
import newuibrowser.di.DatabaseScheduler
import newuibrowser.settings.preferences.DeveloperPreferences
import newuibrowser.settings.preferences.LandscapePreferences
import newuibrowser.settings.preferences.PortraitPreferences
import newuibrowser.settings.preferences.UserPreferences
import newuibrowser.utils.installMultiDex
...
import newuibrowser.settings.Config
import newuibrowser.settings.preferences.ConfigurationPreferences
...
class App : Application(), SharedPreferences.OnSharedPreferenceChangeListener,
    ...
    override fun onActivityDestroyed(activity: Activity) {
        ...
        newuibrowser.utils.MemoryLeakUtils.clearNextServedView(activity, this@App)
    }
    ...
    override fun onCreate() {
        ...
        Thread.setDefaultUncaughtExceptionHandler { thread, ex ->
            if (userPreferences.crashLogs) {
                newuibrowser.utils.FileUtils.writeCrashToStorage(ex)
            }
	...
	// TODO: Remove that once we are done with ReactiveX
        RxJavaPlugins.setErrorHandler { throwable: Throwable? ->
            if (userPreferences.crashLogs && throwable != null) {
                newuibrowser.utils.FileUtils.writeCrashToStorage(throwable)
                throw throwable
            }
        }

        // Apply locale
        val requestLocale = newuibrowser.locale.LocaleUtils.requestedLocale(userPreferences.locale)
        newuibrowser.locale.LocaleUtils.updateLocale(this, requestLocale)
        ...
        newuibrowser.utils.ActivityAliasManager.updateEnabledActivity(this, userPreferences.incomingViewAction)
    ...
    companion object {
         ...
         */
        fun setLocale() {
            val requestLocale = newuibrowser.locale.LocaleUtils.requestedLocale(app.userPreferences.locale)
            newuibrowser.locale.LocaleUtils.updateLocale(app, requestLocale)
        }
